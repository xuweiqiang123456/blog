  ...
  render() {
    ...
    const actions = (v: any, fId: any) => {
      ...
      let del = (
        <Popconfirm
          title="是否要删除此条评论?"
          onConfirm={() => this.delComment(fId, v._id)}
          okText="确定"
          cancelText="取消"
        >
    ...
    const comment = (v: any, children: any, fId: any) => (
      <Comment
        ...
        avatar={
          !v.userinfo.weburl ? (
            ...
            <a
              href={
                !v.userinfo.weburl.split("http")[1]
                  ? "http://" + v.userinfo.weburl
                  : v.userinfo.weburl
              }
              target="_blank"
              rel="noopener"
              title={v.userinfo.weburl}
            >
      ...
      >
        {children
          ? children.map((v: any) => (
              <div key={v._id}>{comment(v, v.children, fId)}</div>
            ))
          : null}
      </Comment>
    );
    ...
    return (
      <div className="CommentList">{list.length > 0 ? listhtml : none}</div>
    );
  }
}
